...
func LoadConfigFromFile(filename string) *Config {
	...
	return &Config{
		...
		TCPConfig: &TcpConfig{
			...
			Sndbuf:       getEnvOrYAMLInt(yamlCfg, "TCP_SNDBUF", "tcp.sndbuf", 65536),
			Rcvbuf:       getEnvOrYAMLInt(yamlCfg, "TCP_RCVBUF", "tcp.rcvbuf", 262144),
			KeepAlive:    true,
			Reader:       getEnvOrYAMLInt(yamlCfg, "TCP_READER", "tcp.reader", 32),
			ReadBuf:      getEnvOrYAMLInt(yamlCfg, "TCP_READBUF", "tcp.read_buf", 1024),
			ReadBufSize:  getEnvOrYAMLInt(yamlCfg, "TCP_READBUF_SIZE", "tcp.read_buf_size", 8192),
			Writer:       getEnvOrYAMLInt(yamlCfg, "TCP_WRITER", "tcp.writer", 32),
			WriteBuf:     getEnvOrYAMLInt(yamlCfg, "TCP_WRITEBUF", "tcp.write_buf", 1024),
			WriteBufSize: getEnvOrYAMLInt(yamlCfg, "TCP_WRITEBUF_SIZE", "tcp.write_buf_size", 8192),
		},
		Protocol: &Protocol{
			Timer:            getEnvOrYAMLInt(yamlCfg, "PROTOCOL_TIMER", "protocol.timer", 32),
			TimerSize:        getEnvOrYAMLInt(yamlCfg, "PROTOCOL_TIMER_SIZE", "protocol.timer_size", 2048),
			SvrProto:         getEnvOrYAMLInt(yamlCfg, "PROTOCOL_SVR_PROTO", "protocol.svr_proto", 1024),
		...
		GettyConfig: &GettyConfig{
			...
			Ports:           getPorts(), // 简化版，只从环境变量读取
			Paths:           []string{getEnvOrYAMLStr(yamlCfg, "GETTY_PATH", "getty.paths[0]", "/connect")},
			ProfilePort:     getEnvOrYAMLInt(yamlCfg, "GETTY_PROFILE_PORT", "getty.profile_port", 10086),
			HeartbeatPeriod: getEnvOrYAMLDuration(yamlCfg, "GETTY_HEARTBEAT_PERIOD_SECONDS", "getty.heartbeat_period", 60*time.Second),
			SessionTimeout:  getEnvOrYAMLDuration(yamlCfg, "GETTY_SESSION_TIMEOUT_SECONDS", "getty.session_timeout", 60*time.Second),
			SessionNumber:   getEnvOrYAMLInt(yamlCfg, "GETTY_SESSION_NUMBER", "getty.session_number", 1000),
			FailFastTimeout: getEnvOrYAMLStr(yamlCfg, "GETTY_FAIL_FAST_TIMEOUT", "getty.fail_fast_timeout", "5s"),
...
// getPorts 从环境变量或默认值获取 Getty 监听端口列表
// 优先级：环境变量 GETTY_PORT > 默认值 "8083"
func getPorts() []string {
	// 首先检查环境变量
	if port := os.Getenv("GETTY_PORT"); port != "" {
		return []string{port}
	}
