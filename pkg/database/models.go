...
type RoomUser struct {
	...
	UserID   string     `gorm:"column:user_id;size:64;not null;index:idx_user_room" json:"user_id"`
	UserName string     `gorm:"column:user_name;size:128;not null" json:"user_name"`
	RoomID   string     `gorm:"column:room_id;size:64;not null;index:idx_user_room" json:"room_id"`
	NodeID   string     `gorm:"column:node_id;size:64;not null" json:"node_id"`
	JoinedAt *time.Time `gorm:"column:joined_at" json:"joined_at,omitempty"`    // 允许 NULL，使用数据库默认值
	LeftAt   *time.Time `gorm:"column:left_at" json:"left_at,omitempty"`        // NULL 表示在线，非 NULL 表示已离线
	IsOnline bool       `gorm:"column:is_online;default:true" json:"is_online"` // 在线状态
}
...
func (ru *RoomUser) BeforeCreate(tx *gorm.DB) error {
	// 注意：JoinedAt 使用数据库默认值 CURRENT_TIMESTAMP
	// 如果需要在应用层设置，可以在这里设置，但通常使用数据库默认值更好
